...
public class TeacherAdminController extends BaseController{
    ...
    @RequestMapping("/exportMystudent")
    public  void exportMystudent(HttpServletRequest request, HttpServletResponse response) throws Exception {
        // 1：初始化数据,得到user表头
        int teacherId=SingletonLoginUtils.getLoginTeaUserId(request);
        ArrayList<String> headlist =excelService.getMyStudentHead();
        ArrayList<ArrayList<String>> dataList = excelService.getMystudent(teacherId);
        String type ="我的学生";
