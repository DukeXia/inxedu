...
public class AdminUserController extends BaseController{
	...
	public ModelAndView queryUserList(HttpServletRequest request,@ModelAttribute("page") PageEntity page){
		ModelAndView model = new ModelAndView();
		model.setViewName(getViewPath("/admin/user/user-list"));
		try{
			QueryUser queryUser =new QueryUser();
			String isavalible=request.getParameter("queryUser.isavalible");
			String schoolId=request.getParameter("shoolId");
			if (StringUtils.isNotEmpty(schoolId)){
				stuClassService.queryCollegeByCollegeId(Integer.parseInt(schoolId));
			}
			String no=request.getParameter("queryUser.no");
			if (StringUtils.isNotEmpty(no)){
				queryUser.setNo(no);
			}
			String s=request.getParameter("queryUser.classId");
			if (StringUtils.isNotEmpty(s)){
				Integer classId=Integer.parseInt(s);
				queryUser.setClassId(classId);

			}
			List<College> list1= collegeService.queryCollege();
			model.addObject("list1",list1);
			if(StringUtils.isEmpty(queryUser.getNo())){

				model.setViewName(getViewPath("/admin/user/user-list"));//学员列表页面
				page.setPageSize(15);
				List<HyberUser> userList = userService.queryUserListPage(queryUser, page);
				model.addObject("userList", userList);
				model.addObject("classId",queryUser.getClassId());
				model.addObject("page", page);
			}else {
				List<HyberUser> list2= userService.queryUserByNo(no,page);
				model.addObject("userList",list2);
				model.addObject("no",queryUser.getNo());
				model.addObject("page",page);
			}
		}catch (Exception e) {
			model.setViewName(this.setExceptionRequest(request, e));
	...
	@RequestMapping("/initPassword/{userId}")
	@ResponseBody
	public String initPassword(@PathVariable("userId") int userId) {
		try {
			User user = userService.queryUserById(userId);
		if(StringUtils.isEmpty(user.getNo())){
			user.setPassword(MD5.getMD5(user.getEmail()));
		}else{
			user.setPassword(MD5.getMD5(user.getNo()));
		}
			userService.updateUserPwd(user);
		} catch (Throwable e) {
			return "error";
	...
	@RequestMapping("/uodateUserInfoPage/{id}")
	public String uodateUserInfoPage(@PathVariable("id")int userId,HttpServletRequest request) throws IOException {
		User user = userService.queryUserById(userId);
		request.setAttribute("user",user);
		return "/inxedu/admin/user/updateUserInfo";
	}
	/**
     * 跳转到发消息页面getuserList
     *
	 ...
	 * @return
	 */
	public List<List<String>> userJoint(List<HyberUser> userList){
	...
	@RequestMapping("/updateStuClassPage/{id}")
	public String updateStuClassPage(@PathVariable("id")int userId,HttpServletRequest request) throws IOException {
		User user = userService.queryUserById(userId);
		request.setAttribute("user",user);
		List<College> list2= collegeService.queryCollege();
		request.setAttribute("list1",list2);
		return "/inxedu/admin/user/updateStuClass";
	}
	@RequestMapping("/updateStuClass/{userId}/{classId}")
	@ResponseBody
	public String  updateStuClass(@PathVariable("userId") int userId,@PathVariable("classId") int classId){
	userService.updateClassIdByUserId(userId,classId);
		return "success";
	}

	@RequestMapping("/updateStuClass")
	public String  updateStuClass(User user){
		userService.updateUserInfo(user);
		return "redirect:/admin/user/getuserList";
	}
	...
	//根据班级名称和学院id模糊查询班级列表
	public ModelAndView search(StuClass stuClass,HttpServletRequest request,PageEntity page,@PathVariable("userId") int userId) {
		User user = userService.queryUserById(userId);
		request.setAttribute("user",user);
		ModelAndView modelAndView=new ModelAndView("/inxedu/admin/user/updateStuClass");
		...
		modelAndView.addObject("stuClassList", list);

		List<College> list2= collegeService.queryCollege();

		modelAndView.addObject("stuClass",stuClass);
		modelAndView.addObject("list1",list2);
		modelAndView.addObject("page",page);
		return modelAndView;
	}
	...
	@RequestMapping("/updateStuClassPage2/{classId}")
	public String updateStuClassPage2(@PathVariable("classId")int classId,HttpServletRequest request,int userId) throws IOException {

		return "/inxedu/admin/user/updateStuClass";
	}
	@RequestMapping("/toAddUser")
	public String toAddUser(HttpServletRequest request){
		List<StuClass> list = stuClassService.queryAllStuclass();
		request.setAttribute("list",list);
			return "/inxedu/admin/user/createUser";
	}
	...
	public Map<String,Object> createUser(HttpServletRequest request,User user){
		...
		Integer userId = userService.queryUserByNoNotPage(user.getNo());
		if(userId!=null){
			json=this.setJson(false,"学号已存在",null);
			return json;
		 }
		 user.setPassword(MD5.getMD5(user.getIdCard()));
		 userService.createUser(user);
		 json=this.setJson(true,"创建学生成功！",null);
		 return json;
	}
}
