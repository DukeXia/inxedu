...
import static com.inxedu.os.common.constants.CacheConstans.USER_TIME_LONG;
...
public class UserController extends BaseController{
	...
	Integer id=	 user.getClassId();
		if(ObjectUtils.isNotNull(id)){
			StuClass s=stuClassService.getStuClassById(id);
	...
	@ResponseBody
	public Map<String,Object> updateUserInfo(HttpServletRequest request,@ModelAttribute("user") HyberUser user){
		Map<String,Object> json = new HashMap<String,Object>();

		try{
		/*	List<Integer> list=stuClassService.queryClassIdByName(user.getClassname());
			String courseId=request.getParameter("courseId");
			if (ObjectUtils.isNotNull(list)){
				user.setClassId(list.get(0));
			}*/
			if(StringUtils.isMobileNo(user.getMobile())){
					if(user.getMobile().equals(userService.getHyberUser(user.getUserId()).getMobile()))
					{
					}
					else {
					...
					json=this.setJson(false,"请确认手机号格式输入正确",null);
				}
				//缓存用户
		}catch (Exception e) {
			this.setAjaxException(json);
	...
	public Map<String,Object> userLogin(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			if("true".equals(ipForget)){
				...
				WebUtils.setCookieOnSeconds(response, CacheConstans.WEB_USER_LOGIN_PREFIX, uuid,(CacheConstans.USER_TIME));
			}else{
				//缓存用户
...
//				System.out.println(CacheConstans.USER_CURRENT_LOGINTIME);
    ...
    public Boolean wechatLogin(HttpServletRequest request,HttpServletResponse response){
        Map<String,Object> json = new HashMap<String,Object>();
        try{
            String account = request.getParameter("account");
            String password = request.getParameter("password");
