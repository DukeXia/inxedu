...
import static com.inxedu.os.common.util.ThreadLocalUtil.dumphreadLocals;
...
public class AppointmentFrontServiceImpl extends GenericDaoImpl implements AppointmentFrontService {
    ...
    public ApponitmentResult appointment(ExamStudent student, Integer examBatchId, Integer examnameId) throws RuntimeException {
        /**
         * 建立日志
         */
        ...
        examAppointmentLog.setEvents("通过id查询考试管理，id=" + examnameId.toString());
        ...
        seat.setState(true);
        Long expect = updateExamSeatBySeatId(seat);
        if (expect < 1L) {
            return new ApponitmentResult(AppointmentState.BUSY);
        }
        /**
         * 准备预约结果数据
         * 
    ...
    public Long updateExamSeatBySeatId(ExamSeat examSeat) {
        Long influence = update("ExamSeatMapper.updateByIdA", examSeat);
        return influence;
    }
