...
public class ExcelServiceImpl extends GenericDaoImpl implements ExcelService {
    ...
    @Override
    public String updateExcelToScore(HttpServletRequest request, MultipartFile myfile) {
        // 返回的结果集
        List<ExamStudentAppointment> examStudentList = new ArrayList<ExamStudentAppointment>();
            ...
            for (int i = 0; i < count; i++) {
                ...
                for (int j = 1; j <= rows + 1; j++) {
                    ...
                    if (row != null) {
                        ...
                        examStudentList.add(examStudentAppointment);
                    }
                }
            }
            if (msg == null || "".equals(msg)) {
                // --------------批量保存
                for (ExamStudentAppointment examStudentAppointment : examStudentList) {
                    this.update("ExamStudentAppointmentMapper.updateScoreBy", examStudentAppointment);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try (InputStream inputStream = myFile.getInputStream();) {
            ...
            if (StringUtils.isNotEmpty(msg)) {
                ...
                for (ExamClassroom examClassroom : list
                    ...
                    this.insert("ExamClassroomMapper.save", examClassroom);
                }
            }
        } catch (Exception e) {
            for (ExamClassroom examClassroom : list
    ...
    public String updateExcelToExamCourse(HttpServletRequest request,
        ...
        try (InputStream inputStream = myFile.getInputStream()) {
            ...
            for (ExamCourse examCourse : list
                ...
                this.insert("ExamCourseMapper.save", examCourse);
            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (StringUtils.isEmpty(msg)) {
                    for (ExamSeat examSeat : list
                        ...
                        this.insert("ExamSeatMapper.save", examSeat);
                    }
                } else {
                    return msg;
                }
    ...
    public String updateExcelToExamStudent(HttpServletRequest request,
        ...
        try (InputStream inputStream = myFile.getInputStream();) {
            ...
            if (msg == null || "".equals(msg)) {
                // --------------批量保存
                for (ExamStudent examStudent : examStudentList) {
                    this.insert("ExamStudentMapper.save", examStudent);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
                       ...
                       /* ExamCourse examCourse=examCourseService.queryCourseByCourseName(coursename);
                        ...
                        if(checkStudentNo(studentNo)){
                            ...
                            if(teacherId!=null&&teacherId>0){
                                ...
                                examStudent.setStudentClass(className);
                             /*   if(examCourse!=null){
