...
$(function () {
    ...
    allQArray.each(function () {
        ...
        N.bind("click", function () {
            ...
            if (!window.needHideShare && !window.hideFriend) {
                if (isWeiXin && !hasClickQ) {
                    pushHistory();
                }
                hasClickQ = true;
            }
        ...
        if (q == "1") {
            ...
            if (P[0]) {
                ...
                k.sendActivitySms = function (ad) {
                    ...
                    $.ajax({
                        type: "GET", url: aa, async: false, success: function (ae) {
                            var af = "";
                            if (ae == "true") {
                ...
                P[0].onchange = P[0].onblur = function () {
                    ...
                    $.ajax({
                        type: "GET", url: aa, async: false, success: function (ad) {
                            ab.issmsvalid = false;
                            var ae = "";
                            if (ad == "true") {
                                ab.issmsvalid = true;
    ...
    $("#ctlNext") != null && $("#ctlNext").on("click", function () {
        ...
        $.ajax({
        type:'POST',
        url: '/front/submitSurveyQuestion',
        data: $("#form1").serialize(),
            success: function (data) {
                if (data.success) {
                    alert(data.message);
                    location.href = "/front/showSurveyList"
                } else {
                    alert(data.message);
                }
            }
...
function updateCart(c) {
    ...
    d.each(function () {
        ...
        var o = p * parseFloat(n).toFixed(2);
        var l = '<li class="productitem"><span class="fpname">' + q + '</span><span class="fpnum">' + p + '</span><span class="fpprice">￥' + o + "</span></li>";
        f += l;
        h += o;
        j += p;
    });
    f = "<ul class='productslist'>" + f + '<li class="productitem"><span class="fpname"></span><span class="fpnum" style="font-weight:bold;">' + j + '</span><span class="fpprice" style="font-weight:bold;">￥' + h.toFixed(2) + "</span></li></ul>";
    $("#shopcart").html(f);
    if (h > 0) {
        $("#shopcart").show();
    } else {
...
function getGender(c, b) {
    ...
    $("input[type='radio']", b).each(function (a) {
        if (this.checked) {
            var e = $(this.parentNode.parentNode).find(".label").html();
            if (e.indexOf("男") > -1) {
                gender = 1;
            } else {
                if (e.indexOf("女") > -1) {
    ...
    $("input[type='radio']", b).each(function (a) {
        if (this.checked) {
            ...
            if (e.indexOf("未婚") > -1) {
                ...
                if (e.indexOf("已婚") > -1 || e.indexOf("离异") > -1) {
                    marriage = 2;
                }
            }
            return false;
...
function getEducation(c, b) {
    if (c != "3") {
        return;
    }
    var d = $("div.field-label", b).html();
    if (d.indexOf("学历") == -1 && d.indexOf("学位") == -1) {
        return;
    }
    if (education) {
        return;
    ...
    $("input[type='radio']", b).each(function (a) {
        if (this.checked) {
            var e = $(this.parentNode.parentNode).find(".label").html();
            if (e.indexOf("硕士") > -1 || e.indexOf("博士") > -1 || e.indexOf("研究生") > -1) {
                education = 5;
            } else {
                if (e.indexOf("本科") > -1) {
                    education = 4;
                } else {
                    if (e.indexOf("大专") > -1 || e.indexOf("专科") > -1) {
                        education = 3;
                    } else {
                        if (e.indexOf("高中") > -1 || e.indexOf("中专") > -1 || e.indexOf("职高") > -1) {
                            education = 2;
                        } else {
                            if (e.indexOf("初中") > -1 || e.indexOf("小学") > -1) {
                                education = 1;
                            }
                        }
                    }
...
function getAge(c, b) {
    ...
    if (c == 3) {
        $("input[type='radio']", b).each(function (a) {
            if (this.checked) {
                e = $(this.parentNode.parentNode).find(".label").html();
                g = a;
                return false;
            }
        });
...
function groupAnswer(n) {
    ...
    if (window.isDianChu) {
        h += "&check_key=" + encodeURIComponent(check_key) + "&check_address=" + encodeURIComponent(check_address) + "&validate_text=dian";
    }
    if (window.cpid) {
        h += "&cpid=" + cpid;
    }
    ...
    if (window.postIframe) {
        ...
        if (k) {
            $.ajax({
                type: "GET", url: h, success: function (e) {
                    afterSubmit(e, n);
                }, error: function () {
            ...
            $.ajax({
                type: "POST", url: h, data: o, dataType: "text", success: function (e) {
                    afterSubmit(e, n);
                }, error: function () {
                    $(".ValError").html("很抱歉，网络连接异常，请重新尝试提交！");
                    $("#ctlNext").show();
...
function checkOnly(e, h) {
    ...
    $.ajax({
        type: "GET", url: d, async: false, success: function (n) {
            if (n == "false1") {
                f = validate_only;
                j.errorOnly[g] = 1;
                if (l.verifycodeinput) {
