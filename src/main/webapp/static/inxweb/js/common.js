...
function resetPassword() {
    ...
    $.ajax({
        url:baselocation+"/index/forgotPassword",
        type:"POST",
        dataType:'json',
        data:{"randomCode":randomCode,"studentNo":studentNo},
...
function lrFun(type) {
    var oBg = $('<div class="bMask"></div>').appendTo($("body")),
        ...
        rlEle = [
            ...
            '<li>' +
            '<input id="u-email" type="text" placeholder="请输入学号" name="" value="" onkeydown="login_js(' + type + ')" /*onkeyup="$(this).next().html(\'\');"*/>' +
            '<p class="lr-tip-wrap"></p>' +
            '</li>' +
            '<li>' +
            '<input id="u-password" type="password" placeholder="请输入密码"  name="" value="" onkeydown="login_js(' + type + ')" /*onkeyup="$(this).next().html(\'\');"*/>' +
            /*'<p class="lr-tip-wrap"><a href ="javascript:forgetPass()" style="float:right">忘记密码？！</a></p>' +*/
            '</li>' +
            '</ol>' +
            /*'<section class="hLh30 of pl10"><span class="fr"><a href="/uc/register" class="c-master fsize12">没有账号？去注册→</a></span>'+*/
            '<section class="hLh30 of1 pl10">' +
            '<span class="fl"><label class="hand c-999 vam"><input type="checkbox" style="vertical-align: -2px;" id="autoThirty">自动登录</label></span></section>' +
            ...
            '<ol class="e-login-options">' +
            '<li>' +
            '<input id="u-email-reg" type="text" placeholder="请输入学号"  name="" value="" onkeyup="$(this).next().html(\'\');">' +
            '<p class="lr-tip-wrap"></p>' +
            '</li>' +
            '<li>' +
...
function dialogLogin(type) {
    ...
    if (userName == "" || userName == null) {
        $("#u-email").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>请输入正确的学号！</span>');
        return false;
    }
    if (pwd == "" || pwd == null) {
...
function dialogRegister() {
    ...
    $.ajax({
        ...
        data: {
            "user.email": $("#u-email-reg").val(), "user.password": $("#u-password-reg").val(),
